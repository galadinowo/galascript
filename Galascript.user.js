...
var nvcsSoftDeactivate, defaultColor;
function nvcsCards() {
    return {0: {key: 'nvcsCardsLoadError', name: 'nvcsCards not loaded.', sprite: '/images/powers/silence.png'}}
}
fetch('https://raw.githubusercontent.com/galadinowo/galascript/refs/heads/main/nvcsCards.js')
    .then(response => {
        if (!response.ok) {
            throw new Error('Failed to load nvcsCards');
        }
        return response.text();
    })
    .then(scriptContent => {
        eval(scriptContent);
        if (typeof nvcsCards === 'function') {
            window.nvcsCards = nvcsCards
        }
    })
    .catch(error => {
        console.log('Failed to load nvcsCards: ', error);
    });

fetch('https://github.com/galadinowo/galascript/raw/refs/heads/main/version.txt')
    .then(response => {
        if (!response.ok) {
    ...
    .then(scriptContent => {
        ...
        plugin.toast({
        title: `${pluginName} update ${scriptContent} detected!`,
        text: ``,
              buttons: [{
                text: "Install",
...
function globalStyles() {
    ...
    style.append('img[src*="images/avatars/A_Hat_for_you.png"] {content:url("https://raw.githubusercontent.com/galadinowo/galascript/refs/heads/main/images/misc/hatsu.png");}')
}
...
function statsHide() {
    ...
    style.append('.vmas-frame.monster .cardFrame {background-image: url("https://raw.githubusercontent.com/galadinowo/galascript/refs/heads/main/images/frames/vmas-frame-spell.png");}')
}
function statsShow() {
...
function updateSoulColor(soul, color) {
    console.log(soulColors[soul], color === soulColors[soul])
    if (color === soulColors[soul]) {
        ...
        style.append(`div:not(.breaking-skin):has(.cardName.${soul}) > .cardImage {background-color: ${color} !important; background-blend-mode: unset; filter: brightness(1);}`);
        style.append(`.${soul}:not(li span):not([onmouseover]) {color: ${color}}`);
    } else {
        style.append(`img[src*="images/souls/${soul}.png"] {filter: drop-shadow(0px 5000px 0 ${color}); transform: translateY(-5000px);}`);
        style.append(`img[src*="images/souls/big/${soul}.png"] {filter: drop-shadow(0px 5000px 0 ${color}); transform: translateY(-5000px);}`);
        style.append(`div:not(.breaking-skin):has(.cardName.${soul}) > .cardImage {background-color: ${color} !important; background-blend-mode: luminosity; filter: brightness(1.1);}`);
        style.append(`.${soul}:not(li span):not([onmouseover]) {color: ${color}}`);
    }
}
...
const frameSpoof = plugin.settings().add({
  ...
  category: 'Card rendering',
  type: "select", options: ["off", "Undertale", "Deltarune", "Time to get serious", "Golden", "Vaporwave", "Spider Party", "Halloween2020", "Christmas2020", "Staff", "Spamton", "Cyber World", "Hollow Knight", "Grimm Troupe", "Void", "FNAFB", "Outbreak", "Mirror Temple", "VMas", "Pokecard 1996"],
  default: "off",
  onChange: (val) => {
  ...
  if (event.target.getAttribute("type") !== 'text') { // if not in a text field
    switch (event.code) {
      ...
      case openGalascriptKeybind?.value():
        newInstall.show();
        break;
      case switchPerspectivesKeybind?.value():
        if (!ingame || !spectate) break;
        window.location.href = `https://undercards.net/Spectate?gameId=${gameId}&playerId=${opponentId}`
        break;
